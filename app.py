...
def init_db():
    ...
    # 3. Таблица Заказов (ОБНОВЛЕННАЯ ПОД R и L + РУЧНОЙ ВВОД)
    db.execute("""CREATE TABLE IF NOT EXISTS orders
                  (id INTEGER PRIMARY KEY AUTOINCREMENT, 
                   customer_name TEXT, 
                   customer_phone TEXT,
                   frame_id INTEGER, 
                   lens_id_right INTEGER, -- ID линзы из склада (R)
                   lens_id_left INTEGER,  -- ID линзы из склада (L)
                   manual_lens_r TEXT,    -- Ручной ввод (R), если нет на складе
                   manual_lens_l TEXT,    -- Ручной ввод (L), если нет на складе
                   pd TEXT,
                   total_price INTEGER, 
                   status TEXT, 
                   date TEXT, 
    ...
    # --- МИГРАЦИИ (Добавление новых колонок в уже созданную базу) ---
    # Для оправ
    try:
    ...
    # Для заказов (добавляем поля для раздельных глаз и ручного ввода)
    columns_to_add = [
        ("lens_id_right", "INTEGER"),
        ("lens_id_left", "INTEGER"),
        ("manual_lens_r", "TEXT"),
        ("manual_lens_l", "TEXT"),
        ("comment", "TEXT"),
        ("is_updated", "INTEGER DEFAULT 0")
    ...
    db.commit()
    db.close()
    print("✅ База данных успешно настроена под спецзаказы!")
